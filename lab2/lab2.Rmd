...
# posterior density of z_13
data.frame("post" = rnorm(n = 1000, 1.75 + 0.85 * (100 - x_mean), 0.66)) %>%  
  ggplot(aes(x = post, y = after_stat(density))) +
  geom_histogram(bins = 100, color = "black", fill = "white")

#probability 
fit_1_c %>% 
  rstan::extract(pars = "y_new") %>%
  as.data.frame() %>% 
  mutate(y_new = ifelse(y_new < 0, 0, 1)) %>% 
  {sum(.$y_new) / 3200}
```

```{r}
# part 1 d 
fit_1_d <- data_aspirin %>% 
  mutate(x = Before - x_mean, y = Before - After) %>%
  {list("N" = 12, "y" = .$y, "x" = .$x)} %>% 
  {stan(
    "model_1_d.stan",
...
```{r}
# for exercise 2 use a package called coda/koda.
```
